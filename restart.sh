...
LOG_DIR="${LOG_DIR:-$PWD/logs/restart}"
LOG_FILE="${LOG_FILE:-$LOG_DIR/restart.log.$(date '+%Y%m%d')}"
...
# 로그 로테이션 (이전 로그 파일 정리)
cleanup_old_logs() {
    find "$LOG_DIR" -name "restart_*.log" -mtime +$MAX_LOG_FILES -delete 2>/dev/null || true
}
...
init_logging() {
    ...
    if [ "$LOG_TO_FILE" = "true" ]; then
        echo "===================================================================================" >> "$LOG_FILE"
        echo "=== SVD Restart Script Started at $(date) ===" >> "$LOG_FILE"
...
# 4단계: SVD 기동
start_supervisord() {
    log_info "[Start] Starting supervisord..."

    # supervisord 실행 파일 확인
    if [ ! -f "$SVD_PATH" ]; then
        log_error "[Start] supervisord executable not found at: $SVD_PATH"
        return 1
    fi

    # 설정 파일 확인
    local conf_file="$PWD/cfg/supervisord.conf"
    if [ ! -f "$conf_file" ]; then
        log_error "[Start] Configuration file not found at: $conf_file"
        return 1
    fi

    log_info "[Start] Starting supervisord with config: $conf_file"

    # supervisord 시작
    if $SVD_PATH 2>&1 | tee -a "$LOG_FILE"; then
        log_info "[Start] Supervisord start command executed"

        # 기동 대기
        log_info "[Start] Waiting ${STARTUP_WAIT}s for supervisord to initialize..."
        sleep $STARTUP_WAIT
        ...
        log_info "[Start] svctl status check"
        {
            echo "============================== svctl status raw output =============================="
            $PWD/svctl status
            echo "====================================================================================="
        ...
        log_warn "[Start] exit_code: $exit_code"

        if [ $exit_code -eq 0 ] || [ $exit_code -eq 3 ]; then
            log_info "[Start] Supervisord status check passed (exit: $exit_code) - Startup successful"
            return 0
        elif [ $exit_code -eq 2 ]; then
            log_error "[Start] Supervisord returned exit code 2 - Invalid command or syntax error"
            return 1
        elif [ $exit_code -eq 4 ]; then
            log_error "[Start] Supervisord returned exit code 4 - Failed to communicate with supervisord (daemon may be down)"
            return 1
        else
            log_error "[Start] Supervisord status check failed (exit: $exit_code)"
            return 1
        fi
    else
        log_error "[Start] Failed to execute supervisord start command"
        return 1
    fi
}
...
# 메인 실행 함수
main() {
    log_info "[Main] Starting restart process for: $SVD_PATH"

    local shutdown_method=""
    local shutdown_success=false

    # 1. 종료 단계
    log_info "[Main] ========== SHUTDOWN PHASE =========="
    ...
    if $shutdown_success; then
        ...
        cleanup
        log_info "[Main] Shutdown phase completed successfully"
    else
        log_error "[Main] Shutdown phase failed"
        return 1
    fi

    # 2. 기동 단계
    log_info "[Main] ========== STARTUP PHASE =========="

    if start_supervisord; then
        log_info "[Main] Startup phase completed successfully"
        log_info "[Main] ========== RESTART COMPLETED =========="
        return 0
    else
        log_error "[Main] Startup phase failed"
        return 1
    fi
}
...
# 시작 환경 정보 로깅
log_info "[Init] SVD Restart Script v1.0"
log_info "[Init] Current user: $(whoami)"
log_info "[Init] Current directory: $PWD"
log_info "[Init] Target SVD path: $SVD_PATH"
log_info "[Init] Timeout settings - Supervisorctl: ${SUPERVISORCTL_TIMEOUT}s, SIGTERM: ${SIGTERM_TIMEOUT}s, Startup wait: ${STARTUP_WAIT}s"
